...
class ComponentCompleter extends GeneralCompleter implements Completer
{
    protected function buildQuestion(string $key, SerializableInventoryItem $in_item): string
    {
        /** @var $in_item Component */
        return "Please fill in the missing value for $key in Component ({$in_item->getTitle()})";
    }

    protected function isMandatory(string $key, SerializableInventoryItem $in_item): bool
    {
        /** @var $in_item Component */
        if ($key === 'roles' && $in_item->getRoles()->get() === []) {
            return true;
        }
        return in_array($key, ['testing_needed']);
    }

    protected function completeItemCollection(AbstractInventoryItemCollection $c): AbstractInventoryItemCollection
    {
        return parent::completeItemCollection($c);
    }
